class Slide {
  constructor(node) {
    ...
    Array.from(node.querySelectorAll('*')).forEach(child => {
      ...
      if (highlight) {
        this.maxOrder = Math.max(this.maxOrder, parseInt(highlight));
      }
      ...
      if (child.classList.contains('stack')) {
        child.style.transform = `translate(-50%, -50%) rotate(${(Math.random() * 8 - 4).toFixed(2)}deg)`;
        child.style.zIndex = ++stackOrder;
      }
    });
  }
  ...
  update() {
    ...
    Array.from(this.node.querySelectorAll('video[autoplay]')).forEach(child => {
      child.currentTime = 0;
      child.play();
    })
  }
